...
static int tcs_rgb_data_read (struct i2c_client *client, rgb_data_t *data) {
    ...
    uint8_t status;
    int rv;
    rv = tcs_read_registers(client, REGISTER_ADDRESS_STATUS, &status, 1);
    pr_info("Status value: %d \n", status);
...
static long hfn_tcs_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    {
        case TCS_GET_COLORS:
            tcs_rgb_data_read(hfn_i2c_client_tcs, &rgb_data);
...
static int hfn_tcs_probe(struct i2c_client *client, const struct i2c_device_id *id) {
    ...
    rv = tcs_init(client);
    if (rv < 0) return rv;
    tcs_rgb_data_read(client, &read_rgb);

    pr_info("Clear data: %d \n", read_rgb.clear_data);
    pr_info("Red data: %d \n", read_rgb.red_data);
    pr_info("Green data: %d \n", read_rgb.green_data);
    pr_info("Blue data: %d \n", read_rgb.blue_data);
